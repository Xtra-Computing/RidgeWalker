...
public:
    ...
    std::vector<uint64_t>  simulate_all_size(uint32_t seed, uint32_t mask, uint32_t num_of_stream)
    {
        TRACE();
        std::vector<uint64_t> ground_truth;
        for (auto i = 0; i < num_of_stream; i ++)
        {
            uint64_t value = 0;
    ...
    std::vector<uint64_t>  simulate_sink_res(uint32_t seed, uint32_t mask, uint32_t num_of_stream)
    {
        std::vector<uint64_t> ground_truth;
        for (auto i = 0; i < num_of_stream; i ++)
        {
            uint64_t init = 0;
            ground_truth.push_back(init);
        }
        ...
        {
            uint32_t rng = minRand(31, 0);
            uint32_t vid = (rng & this->mask);
            uint32_t channel_index = (rng & mask);


            vertex_descriptor_t v;
            v.start =  vid >> 4;
            v.size  =  (channel_index) + ((vid >> 3) & 0x1) * num_of_stream;


            uint64_t *p_inc = (uint64_t *) &v;
            uint64_t ori = ground_truth[channel_index];

            ori += *p_inc;

            ground_truth[channel_index] = ori;
