...
template <typename T>
void rate_limiter ( const  int          div_mask,
                    hls::stream< T >    &input,
                    hls::stream< T >    &output
                ) {
...
template <int A, int B, typename stream_t, typename pkg_t>
void axis_inject_delay ( stream_t  &input,
                         stream_t  &output
                  ) {
#pragma HLS interface ap_ctrl_none port=return
    uint32_t counter = 0;
    volatile uint8_t input_valid_flag = 0;
    while (true) {
#pragma HLS PIPELINE II=1
        pkg_t  pkg;
