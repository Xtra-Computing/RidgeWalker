...
void debug_dump(  debug_msg_inner_stream_t (&in)[NUM_OF_STREAM], ap_uint<64>   *mem )
{
    debug_msg_item_t dump_array[NUM_OF_STREAM * 2];
#pragma HLS ARRAY_PARTITION variable=dump_array   dim=0
    ...
    for (int i = 0; i < NUM_OF_STREAM; i++)
    {
        dump_array[i + NUM_OF_STREAM] = in[i].read();
    }

    for (int i = 0; i < (NUM_OF_STREAM * 2); i ++)
    {
        mem[i] = dump_array[i];
    }
...
#pragma HLS dataflow
        ...
        debug_msg_inner_stream_t debug_stream[NUM_OF_STREAM];
