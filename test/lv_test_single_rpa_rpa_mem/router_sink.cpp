...
extern "C" {
    void router_sink( ap_uint<128>          *mem,
                      uint32_t              size,
                      rpa_task_stream_t      &input
                    )
    {
#pragma HLS INTERFACE m_axi port=mem offset=slave bundle=gmem num_write_outstanding=256  max_write_burst_length=32


        for (int j = 0; j < size; j += 32) {
            for (int i = 0; i < 32; i ++) {
#pragma HLS pipeline
                rpa_task_pkg_t pkg = input.read();
                mem[j + i] =  pkg.data;
            }
        }
