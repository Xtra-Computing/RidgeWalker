#ifndef __RNG_QUERY_LOADER_H__
#define __RNG_QUERY_LOADER_H__
...
#define MODE_SET_RNG_QUERY      (2)
#define MODE_SET_MAPPER         (1)
#define MODE_SET_QUERY          (0)



class rng_config_loader: public cl_krnl
{
public:
    rng_config_loader() {}
    uint32_t seed;
    ...
    rng_config_loader(cl_accelerator &acc, uint32_t id)
        : cl_krnl(acc, "rng_config_loader", id)
    {
    ...
    int init_rng_query(query_vector_t  &query, uint32_t query_len)
    {

        return init_rng_query(query.size(), query_len);

    }

    int init_rng_query(int size, int query_len)
    {
        TRACE()
        ...
        for (int i = 0; i < 8; i ++)
        {
            query.push_back(i % (1024 * 4));
        }

        DEBUG_PRINTF("size: %d", 8 * sizeof(uint64_t));
        HOST_BUFFER(query, 8 * sizeof(uint64_t));
        OCL_CHECK(err, err = krnl.setArg(0, buffer_query));

        OCL_CHECK(err, err = mem_q.enqueueMigrateMemObjects({   buffer_query,
                                                            }, 0 /*0 means from host*/));

        uint32_t  num_query_x8 = size / 8;
        uint32_t  mode  = MODE_SET_RNG_QUERY;
