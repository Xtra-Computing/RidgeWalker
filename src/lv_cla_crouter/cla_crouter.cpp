...
void cla_to_net_with_timeout(   route_vertex_frp_inner_stream_t &in,
...
coalease: while (true)
    {
        ...
        if (in.read_nb(in_pkg)) {
            wait_counter = 0;
            cla_task_item_t cla_task = in_pkg.ap_member(route_vertex_t, data);
            buffer.ap_member_array(route_net_vertex_t, data, buffer_counter) = cla_task;
...
#pragma HLS unroll
                    if (mask & (1 << i))
                    {
                        cla_task_item_t cla = net_data.ap_member_array(route_net_vertex_t, data, i);
                        lcolumn_access_item_t v = cla.ap_cla_ca();

                        route_vertex_frp_item_t  rv;

                        rv.ap_member(route_vertex_t, dest) = v.ap_cl_dest() >> 1;
                        rv.ap_member(route_vertex_t, data) = cla;
...
#pragma HLS DEPENDENCE variable=map_array type=inter false
    ...
    while (1) {
        ...
        if (stream_in.read_nb(frp_item))
        {
            ...
            //
            cla_task_item_t cla = frp_item.ap_member(struct_t, data);
            lcolumn_access_item_t v = cla.ap_cla_ca();
            //

            ap_uint<3> old_id = v.ap_cl_dest() >> 4;
...
#pragma HLS stream variable=l2_switch  depth=31
        ...
        cla_input< FRP_TEMPLATE(route_vertex) >(in1, in_stream[0]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in2, in_stream[1]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in3, in_stream[2]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in4, in_stream[3]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in5, in_stream[4]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in6, in_stream[5]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in7, in_stream[6]);
        cla_input< FRP_TEMPLATE(route_vertex) >(in8, in_stream[7]);
...
#pragma HLS unroll
        ...
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[0], out1);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[1], out2);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[2], out3);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[3], out4);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[4], out5);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[5], out6);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[6], out7);
        cla_output< FRP_TEMPLATE(route_vertex) >(out_stream[7], out8);
        ...
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[0], rout1);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[1], rout2);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[2], rout3);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[3], rout4);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[4], rout5);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[5], rout6);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[6], rout7);
        //cla_output< FRP_TEMPLATE(route_vertex) >(rout_stream[7], rout8);
...
#pragma HLS unroll
            cla_to_net_with_timeout(rout_stream[i], coaleased_frame[i], ((const int )(i + 1)), __NET_FRAME_SIZE__, __TIMEOUT__);
        }
