...
public:
    ...
    {
        uint32_t timeout_counter = max_size;
#ifdef __EMULATION__

#define  MIN_TIMEOUT_COUNTER (16384 * 4)
        timeout_counter= MIN_TIMEOUT_COUNTER;
#else
        timeout_counter = ((16384 * 128 * 8)> max_size)?(16384 * 128 * 8):max_size;
#endif
        DEBUG_PRINTF("using %d for timeout", timeout_counter);
