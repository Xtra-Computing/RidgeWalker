...
typedef ap_axiu<(sizeof(name##_t) * 8), 1, 1, 16>           name##_net_pkg_t;               \
typedef hls::stream< name##_net_pkg_t >                     name##_net_stream_t;
...
typedef ap_axiu<512, 1, 1, 16>              net_pkg_t;
typedef hls::stream< net_pkg_t >            net_stream_t;

typedef ap_uint<512>                        net_item_t;
typedef hls::stream< net_item_t >           net_inner_stream_t;
typedef ap_uint<513>                        net_frp_item_t;
typedef hls::stream< net_frp_item_t >       net_frp_inner_stream_t;
...
GEN_STRUCT_INTERFACE(net_frame)
...
GEN_STRUCT_INTERFACE(net_signal)


// reserve additional 8 bit for advance features
#define mem_access_item_t(N)                ap_uint< (N + 8) >
#define mem_access_inner_stream_t(N)        hls::stream< mem_access_item_t(N) >
#define mem_access_pkg_t(N)                 ap_axiu< (N + 8), 0, 0, 0>
#define mem_access_stream_t(N)              hls::stream< mem_access_pkg_t(N) >
#define mem_access_frp_item_t(N)            ap_uint< (N + 8) + 1>
#define mem_access_frp_inner_stream_t(N)    hls::stream< mem_access_frp_item_t(N) >

#define ap_mem_data(st)                range(st-1 , 0)
...
GEN_STRUCT_INTERFACE(debug_msg)
