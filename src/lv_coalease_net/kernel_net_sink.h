#ifndef __NET_SINK_H__
#define __NET_SINK_H__
...
public:
    ...
    net_sink(cl_accelerator &acc, uint32_t id)
        : cl_krnl(acc, "net_sink", id)
    {
    ...
    int mem_transfer(  std::vector<uint128_t, aligned_allocator<uint128_t> >  & data)
    {
        ...
        TRACE()
        HOST_BUFFER(data, data.size() * sizeof(uint64_t));
