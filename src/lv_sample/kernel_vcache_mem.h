...
class vcache_mem: public cl_krnl
{

public:
    vcache_mem() {}

    vcache_mem(cl_accelerator &acc, uint32_t gid, uint32_t id)
        : cl_krnl(acc, "vcache_mem", gid, id, 1)
    {

    }


    int init(vertex_descriptor_vector_t  &row_index, uint32_t arg_index)
    {
        TRACE()
