...
#include "load_graph.h"
    ...
    for (int iteration = 0; iteration < 2; iteration ++)
    {
        ...
        krnl_fetch_next.async_schedule_task();
        krnl_ring_manager.async_schedule_task();
        krnl_vertex_loader.async_schedule_task();
        krnl_step_counter.async_schedule_task();



        start = getCurrentTimestamp();
        krnl_step_loader.async_schedule_task();

        krnl_ring_manager.async_join();
        krnl_step_counter.async_join();
        krnl_vertex_loader.async_join();
        krnl_fetch_next.async_join();
        krnl_step_loader.async_join();
        //pthread_exit(0);
